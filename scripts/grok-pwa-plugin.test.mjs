...
const TEST_WORKSPACE = mkdtempSync(join(tmpdir(), "grok-pwa-workspace-"));

function injectGrokPwaHead(html, ctx = {}) {
  return injectGrokPwaHeadImpl(html, { cwd: TEST_WORKSPACE, ...ctx });
}

function createHeadInjector(ctx = {}) {
  return createHeadInjectorImpl({ cwd: TEST_WORKSPACE, ...ctx });
}
